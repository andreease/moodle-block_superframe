...
function block_superframe_display_in_table($data)
{
    ...
    foreach ($data as $onedata) {
        ...
        $editlink = html_writer::link(new moodle_url($baselink, [
            'id' => $onearray['id'],
            'action' => 'edit'
        ]), get_string('edit'));
        $rowdata[] = $editlink;

        $deletelink = html_writer::link(new moodle_url($baselink, [
            'id' => $onearray['id'],
            'action' => 'delete'
        ]), get_string('delete'));
        $rowdata[] = $deletelink;
...
if ($action == "edit") {
    ...
    $filter = array();
    /* Use course id 'if ($courseid != $SITE->id) {
        $filter['id'] = $courseid;
    }' */
